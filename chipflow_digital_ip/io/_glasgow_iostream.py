    ...
    return data.StructLayout(
        {
            name: fn(item[1])
            for name, item in ioshape.items()
            if _filter_ioshape(direction, item)
        }
    )


class FakeFFBuffer(io.FFBuffer):
    def elaborate(self, platform):
        m = Module()
        if self.direction is not io.Direction.Output:
            i_ff = Signal(len(self.port.i), reset_less=True)
            m.d[self.i_domain] += i_ff.eq(self.port.i)
            m.d.comb += self.i.eq(i_ff)

        if self.direction is not io.Direction.Input:
            o_ff = Signal(len(self.port.o), reset_less=True)
            oe_ff = Signal(reset_less=True)
            m.d[self.o_domain] += o_ff.eq(self.o)
            m.d[self.o_domain] += oe_ff.eq(self.oe)
            m.d.comb += self.port.o.eq(o_ff)
            m.d.comb += self.port.oe.eq(oe_ff)
        return m
...
class IOStreamer(wiring.Component):
            ...
            data.StructLayout(
                {
                    "port": _map_ioshape(
                        "o",
                        ioshape,
                        lambda width: data.StructLayout(
                            {
                                "o": width
                                if ratio == 1
                                else data.ArrayLayout(width, ratio),
                                "oe": 1,
                            }
                        ),
                    ),
    ...
    def __init__(self, ioshape, ports, /, *, ratio=1, init=None, meta_layout=0):
        ...
        super().__init__(
            {
                "o_stream": In(
                    self.o_stream_signature(
                        ioshape, ratio=ratio, meta_layout=meta_layout
                    )
                ),
                "i_stream": Out(
                    self.i_stream_signature(
                        ioshape, ratio=ratio, meta_layout=meta_layout
                    )
                ),
            }
    ...
    def elaborate(self, platform):
        ...
        if self._ratio == 1:
            buffer_cls, latency = FakeFFBuffer, 1
        if self._ratio == 2:
        ...
        if isinstance(self._ports, PortGroup):
            ...
            for name, sub_port in self._ports.__dict__.items():
                ...
                m.submodules[f"buffer_{name}"] = buffer[name] = buffer_cls(
                    direction, sub_port
                )
        ...
        with m.If(self.o_stream.valid & self.o_stream.ready):
            for buffer_parts, stream_parts in _iter_ioshape(
                "o", self._ioshape, buffer, self.o_stream.p.port
            ):
                m.d.comb += buffer_parts.o.eq(stream_parts.o)
                m.d.comb += buffer_parts.oe.eq(stream_parts.oe)
            for latch_parts, stream_parts in _iter_ioshape(
                "o", self._ioshape, o_latch, self.o_stream.p.port
            ):
                if self._ratio == 1:
        ...
        with m.Else():
            for buffer_parts, latch_parts in _iter_ioshape(
                "o", self._ioshape, buffer, o_latch
            ):
                if self._ratio == 1:
        ...
        i_en = delay(
            self.o_stream.valid & self.o_stream.ready & self.o_stream.p.i_en,
            name="i_en",
        )
        ...
        skid = Array(
            Signal(self.i_stream.payload.shape(), name=f"skid_{stage}")
            for stage in range(1 + latency)
        )
        for skid_parts, buffer_parts in _iter_ioshape(
            "i", self._ioshape, skid[0].port, buffer
        ):
...
class IOClocker(wiring.Component):
                        ...
                        lambda width: data.StructLayout(
                            {
                                "o": width
                                if _ratio == 1
                                else data.ArrayLayout(width, _ratio),
                                "oe": 1,
                            }
                        ),
                    ),
    ...
    @staticmethod
    def o_stream_signature(ioshape, /, *, ratio=1, meta_layout=0):
        return IOStreamer.o_stream_signature(
            ioshape, ratio=ratio, meta_layout=meta_layout
        )
    ...
    def __init__(self, ioshape, *, clock, o_ratio=1, meta_layout=0, divisor_width=16):
        ...
        super().__init__(
            {
                "i_stream": In(
                    self.i_stream_signature(ioshape, meta_layout=meta_layout)
                ),
                "o_stream": Out(
                    self.o_stream_signature(
                        ioshape, ratio=o_ratio, meta_layout=meta_layout
                    )
                ),
    ...
    def elaborate(self, platform):
        ...
        for i_parts, o_parts in _iter_ioshape(
            "io", self._ioshape, self.i_stream.p.port, self.o_stream.p.port
        ):
