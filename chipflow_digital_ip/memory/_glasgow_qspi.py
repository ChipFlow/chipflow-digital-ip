    ...
    def elaborate(self, platform):
        ...
        with m.Switch(self.octets.p.mode):
            ...
            with m.Case(QSPIMode.PutX2):
                m.d.comb += Cat(self.frames.p.port.io1.o, self.frames.p.port.io0.o).eq(
                    rev_data.word_select(cycle, 2)
                )
                m.d.comb += Cat(
                    self.frames.p.port.io1.oe, self.frames.p.port.io0.oe
                ).eq(0b11)
            with m.Case(QSPIMode.GetX2):
                m.d.comb += self.frames.p.i_en.eq(1)
            with m.Case(QSPIMode.PutX4):
                m.d.comb += Cat(
                    self.frames.p.port.io3.o,
                    self.frames.p.port.io2.o,
                    self.frames.p.port.io1.o,
                    self.frames.p.port.io0.o,
                ).eq(rev_data.word_select(cycle, 4))
                m.d.comb += Cat(
                    self.frames.p.port.io3.oe,
                    self.frames.p.port.io2.oe,
                    self.frames.p.port.io1.oe,
                    self.frames.p.port.io0.oe,
                ).eq(0b1111)
    ...
    def elaborate(self, platform):
        ...
        with m.Switch(self.frames.p.meta):
            with m.Case(QSPIMode.GetX1, QSPIMode.Swap):  # note: samples IO1
                m.d.comb += self.octets.p.data.eq(
                    Cat(self.frames.p.port.io1.i, data_reg)
                )
            with m.Case(QSPIMode.GetX2):
                m.d.comb += self.octets.p.data.eq(
                    Cat(self.frames.p.port.io0.i, self.frames.p.port.io1.i, data_reg)
                )
            with m.Case(QSPIMode.GetX4):
                m.d.comb += self.octets.p.data.eq(
                    Cat(
                        self.frames.p.port.io0.i,
                        self.frames.p.port.io1.i,
                        self.frames.p.port.io2.i,
                        self.frames.p.port.io3.i,
                        data_reg,
                    )
                )
...
class QSPIController(wiring.Component):
    def __init__(self, ports, *, chip_count=1, use_ddr_buffers=False):
        assert len(ports.sck) == 1 and ports.sck.direction in (
            io.Direction.Output,
            io.Direction.Bidir,
        )
        assert len(ports.io) == 4 and ports.io.direction == io.Direction.Bidir
        assert len(ports.cs) >= 1 and ports.cs.direction in (
            io.Direction.Output,
            io.Direction.Bidir,
        )
        ...
        super().__init__(
            {
                "o_octets": In(
                    stream.Signature(
                        data.StructLayout(
                            {"chip": range(1 + chip_count), "mode": QSPIMode, "data": 8}
                        )
                    )
                ),
    ...
    def elaborate(self, platform):
        ...
        m.submodules.io_clocker = io_clocker = IOClocker(
            ioshape, clock="sck", o_ratio=ratio, meta_layout=QSPIMode
        )
