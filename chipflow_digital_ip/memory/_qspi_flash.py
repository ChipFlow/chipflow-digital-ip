...
class WishboneQSPIFlashController(wiring.Component):
    ...
    def __init__(self, *, addr_width, data_width):
        super().__init__(
            {
                ...
                "wb_bus": In(
                    wishbone.Signature(
                        addr_width=addr_width, data_width=data_width, granularity=8
                    )
                ),
                "spi_bus": Out(
                    wiring.Signature(
        ...
        self.wb_bus.memory_map = MemoryMap(
            addr_width=addr_width + exact_log2(data_width // 8), data_width=8
        )
    ...
    def elaborate(self, platform):
        ...
        with m.FSM() as fsm:
            ...
            with m.State("SPI-Address"):
                ...
                m.d.comb += self.spi_bus.o_octets.p.data.eq(
                    flash_addr.word_select(o_addr_count, 8)
                )
                m.d.comb += self.spi_bus.o_octets.valid.eq(1)
                with m.If(self.spi_bus.o_octets.ready):
                    ...
                    with m.Elif(self._config.f.dummy_bytes.data != 0):
                        m.d.sync += o_dummy_count.eq(
                            self._config.f.dummy_bytes.data - 1
                        )
            ...
            with m.State("SPI-Data-Read"):
                ...
                with m.If(self.spi_bus.i_octets.valid):
                    m.d.sync += self.wb_bus.dat_r.word_select(i_data_count, 8).eq(
                        self.spi_bus.i_octets.p.data
                    )
            ...
            with m.State("Raw-Wait"):
                with m.If(~self._config.f.raw_enable.data):
                    ...
                    m.next = "SPI-Deselect"
                with m.Elif(
                    self._raw_control.f.deselect.w_stb
                    & self._raw_control.f.deselect.w_data
                ):
                    m.next = "Raw-Deselect"
...
class QSPIFlash(wiring.Component):
    ...
    def __init__(self, *, addr_width, data_width):
        ...
        self._ctrl = WishboneQSPIFlashController(
